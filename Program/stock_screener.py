...
from helper_functions import modify_current_date, generate_end_dates, get_currency, get_df, get_excel_filename, get_infix, get_rs, get_volume_sma_ranks, get_stock_info, stock_market
import itertools
...
def check_conds_tech(index_name, df):
    ...
    conds_tech = cond_t1 and cond_t3 and cond_t4 and cond_t5
...
def process_stock(stock, index_name, index_df, end_date, current_date, stock_dfs, stock_infos, rs_volume_df_short, rs_volume_df_long, backtest=False):
    """
    ...
    - rs_volume_df_short (DataFrame): DataFrame for short-term relative strength and volume.
    - rs_volume_df_long (DataFrame): DataFrame for long-term relative strength and volume.
        ...
        if conds_tech:
            ...
            if conds_tech and cond_f1:
                ...
                if conds_fund:
                    ...
                    # Calculate alpha and beta of the stock
                    alpha = calculate_alpha(df, index_df)
                    beta = calculate_beta(df, index_df)
...
def select_stocks(end_dates, current_date, index_name, index_dict, 
                  period_short, period_long, RS, all_stocks, factors, backtest):
    """
    ...
    - period_short (int): The short period for analysis in days.
    - period_long (int): The long period for analysis in days.
    ...
    # Add this check to exit early if no dates remain
    if len(end_dates) == 0:
        print(f"All files already exist for {index_name}. Skipping stock selection for these dates.")
        return

    # Get current stocks in the market
    stocks_current = stock_market(current_date, current_date, index_name, all_stocks)
    ...
    for end_date in tqdm(end_dates, desc="Processing end dates"):
        ...
        stocks = stock_market(end_date, current_date, index_name, all_stocks)
        ...
        index_return_short = (index_df["Percent Change"] + 1).tail(period_short).cumprod().iloc[-1]
        index_return_long = (index_df["Percent Change"] + 1).tail(period_long).cumprod().iloc[-1]
        index_shortName = index_dict[f"{index_name}"]
        print(f"Return for {index_shortName} between {index_df.index[-period_long].strftime('%Y-%m-%d')} and {end_date}: {index_return_long:.2f}")

        # Create relative strength and volume dataframes
        rs_dfs, volume_dfs, rs_volume_dfs = create_rs_volume_df(stocks, dfs, [end_date, end_date], [period_short, period_long], [index_return_short, index_return_long], index_shortName, result_folder, infix, backtest)
        rs_df_short, rs_df_long = rs_dfs
        volume_df_short, volume_df_long = volume_dfs
        rs_volume_df_short, rs_volume_df_long = rs_volume_dfs
        
        # Filter stocks based on criteria
        if index_name == "^HSI":
            rs_df_long = rs_df_long[rs_df_long["RS"] >= RS]
            volume_df_long = volume_df_long[(volume_df_long["Volume SMA 5 Rank"] <= 800) | (volume_df_long["Volume SMA 20 Rank"] <= 800)]
            rs_stocks = set(rs_df_long["Stock"])
            volume_stocks = set(volume_df_long["Stock"])
            stocks = list(rs_stocks.intersection(volume_stocks))
        else:
            rs_df_long = rs_df_long[rs_df_long["RS"] >= RS]
            stocks = rs_df_long["Stock"]
        ...
        export_data = [process_stock(stock, index_name, index_df, end_date, current_date, stock_dfs, stock_infos, rs_volume_df_short, rs_volume_df_long, backtest=backtest) for stock in tqdm(stocks)]
        ...
        # Skip the end_date if no stock is selected
        if stocks_num == 0:
            continue
...
def create_stock_dict(end_dates, index_name, index_dict, all_stocks, factors, RS=90, period=252, cap_threshold=1, backtest=False, recreate_dict=False):
    """
    ...
    if not os.path.isfile(stock_dict_filename) or recreate_dict:
        ...
        for end_date in end_dates[:-1]:
            ...
            if os.path.isfile(filename):
                ...
                # If the file does not exist, set df to None
                df = None
    
            # Extract the number of stocks
            stocks_num = df.shape[0] if df is not None else 0
