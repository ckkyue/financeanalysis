...
// Inputs
// MA length and type
ma1Length = input.int(title = 'MA 1 Length', defval = 5, minval = 1, group = 'Moving Averages', inline = '1')
ma1Type = input.string(title = 'MA 1 Type', defval = 'SMA', options = ['SMA', 'EMA'], group = 'Moving Averages', inline = '1')
ma2Length = input.int(title = 'MA 2 Length', defval = 10, minval = 1, group = 'Moving Averages', inline = '2')
ma2Type = input.string(title = 'MA 2 Type', defval = 'SMA', options = ['SMA', 'EMA'], group = 'Moving Averages', inline = '2')
ma3Length = input.int(title = 'MA 3 Length', defval = 20, minval = 1, group = 'Moving Averages', inline = '3')
ma3Type = input.string(title = 'MA 3 Type', defval = 'SMA', options = ['SMA', 'EMA'], group = 'Moving Averages', inline = '3')
ma4Length = input.int(title = 'MA 4 Length', defval = 50, minval = 1, group = 'Moving Averages', inline = '4')
ma4Type = input.string(title = 'MA 4 Type', defval = 'SMA', options = ['SMA', 'EMA'], group = 'Moving Averages', inline = '4')
ma5Length = input.int(title = 'MA 5 Length', defval = 100, minval = 1, group = 'Moving Averages', inline = '5')
ma5Type = input.string(title = 'MA 5 Type', defval = 'SMA', options = ['SMA', 'EMA'], group = 'Moving Averages', inline = '5')
ma6Length = input.int(title = 'MA 6 Length', defval = 200, minval = 1, group = 'Moving Averages', inline = '6')
ma6Type = input.string(title = 'MA 6 Type', defval = 'SMA', options = ['SMA', 'EMA'], group = 'Moving Averages', inline = '6')

// Color for moving averages (MA) and volume plot
...
plot(closeMa1, color=ma1Color)

closeMa2 = get_moving_average(close, ma2Length, ma2Type)
plot(closeMa2, color=ma2Color)

closeMa3 = get_moving_average(close, ma3Length, ma3Type)
plot(closeMa3, color=ma3Color)

closeMa4 = get_moving_average(close, ma4Length, ma4Type)
plot(closeMa4, color=ma4Color)

closeMa5 = get_moving_average(close, ma5Length, ma5Type)
plot(closeMa5, color=ma5Color)

closeMa6 = get_moving_average(close, ma6Length, ma6Type)
plot(closeMa6, color=ma6Color)

// MVP and VCP conditions
periodMVP = input.int(title="MVP Period", defval=15, minval=1, group = 'MVP & VCP', inline = '1')
periodVCP = input.int(title="VCP Period", defval=10, minval=1, group = 'MVP & VCP', inline = '2')
contraction = input.float(title="Contraction", defval=0.05, minval=0.01, group = 'MVP & VCP', inline = '2')

// Check the M condition
greater = close > close[1] ? 1 : 0
MSum = math.sum(greater, periodMVP)
M = MSum >= (periodMVP * 0.8)

// Check the V condition
V = volume >= volume[periodMVP] * 1.2

// Check the P condition
P = close >= close[periodMVP] * 1.2

// Initialise MVP label
var string mvpLabel = na

// Define MVP labels based on conditions
if M and not V and not P
    mvpLabel := "M"
else if M and V and not P
    mvpLabel := "MV"
else if M and P and not V
    mvpLabel := "MP"
else if M and V and P
    mvpLabel := "MVP"
else
    mvpLabel := na

// Plot MVP conditions
plotshape(mvpLabel == "M", title = 'M', style=shape.triangleup, location=location.abovebar, color=color.gray, size=size.tiny)
plotshape(mvpLabel == "MV", title = 'MV', style=shape.triangleup, location=location.abovebar, color=color.blue, size=size.tiny)
plotshape(mvpLabel == "MP", title = 'MP', style=shape.triangleup, location=location.abovebar, color=color.yellow, size=size.tiny)
plotshape(mvpLabel == "MVP", title = 'MVP', style=shape.triangleup, location=location.abovebar, color=color.green, size=size.tiny)

// Calculate VCP condition
HC = ta.highest(close, periodVCP)
LC = ta.lowest(close, periodVCP)
VCP = na(HC) or na(LC) or HC == 0 ? false : (1 - LC / HC) <= contraction

// Plot VCP condition
plotchar(VCP, "VCP", ">", location.abovebar, color=color.orange, size=size.tiny)
...
i_BorderColor = input(color.black, title = '| Color', group = 'Table', inline = '4')
i_ResultBackgroundColorOdd = input(color.silver, title = 'Odd Rows', group = 'Table', inline = '5')
i_ResultBackgroundColorEven = input(color.white, title = 'Even Rows', group = 'Table', inline = '5')

// Digit settings
i_Estimates = input(true, title = 'Estimates', group = 'Digit', inline = '1')
i_Compare = input(false, title = 'Show VS YoY', group = 'Digit', inline = '2')
i_YoY = input(true, title = 'YoY', group = 'Digit', inline = '2')
i_QoQ = input(false, title = 'QoQ', group = 'Digit', inline = '2')
i_Surprises = input(false, title = '% Surprise', group = 'Digit', inline = '3')
i_PosSurp = input(color.teal, title = '+ve', group = 'Digit', inline = '3')
i_NegSurp = input(color.red, title = '-ve', group = 'Digit', inline = '3')
i_GrossMargin = input(false, title = 'Gross Margin', group = 'Digit', inline = '4')
i_ROE = input(false, title = 'Return On Equity', group = 'Digit', inline = '4')
i_RowAndColumnTextColor = input(color.black, title = 'Row & Column Text Color', group = 'Digit', inline = '5')
i_PosColor2 = input(color.blue, title = '% Positive', group = 'Digit', inline = '6')
i_NegColor2 = input(color.red, title = '% Negative', group = 'Digit', inline = '6')
...
// Get estimates of next earning from TradingView
...
// Replace missing EPS by standard EPS when available
if na(actualEPS)
...
// Check if the sales change is good
...
// Function to define previous quarters gross margin (GM) and return on equity (ROE)
f_grossMargin(i) =>
...
// Calculate EPS YoY change on same quarter
...
// Check whether the calculation has been done with a previous negative EPS, display "#" if yes                                                                                 // added this condition because 0.98 vs -0.16 = #712/713% not 999% APA
...
// If sales > 1000M, display it in B
